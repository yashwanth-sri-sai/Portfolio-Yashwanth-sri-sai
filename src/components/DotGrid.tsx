...
const DotGrid = ({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const dot of dotsRef.current) {
        ...
        ctx.beginPath();
        ctx.arc(ox, oy, rDiv2, 0, Math.PI * 2);
  ...
  }, [proximity, baseColor, activeRgb, baseRgb, tension, friction, dotSize]);
    ...
    const onMove = (e: MouseEvent) => {
      ...
      for (const dot of dotsRef.current) {
        ...
        if (speed > speedTrigger && dist < proximity) {
          // Push away based on velocity (inertia)
          dot.vx += vx * 0.003;
          dot.vy += vy * 0.003;
        }
      }
    ...
    const onClick = (e: MouseEvent) => {
      ...
      for (const dot of dotsRef.current) {
        ...
        if (dist < shockRadius) {
          ...
          const pushX = (dot.cx - cx) * shockStrength * falloff * 0.2;
          const pushY = (dot.cy - cy) * shockStrength * falloff * 0.2;
          dot.vx += pushX;
          dot.vy += pushY;
        }
      }
